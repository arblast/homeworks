...
class MetaCorgiPerkPackage
  ...
  #   type = name.to_s
  #   type_display = type.capitalize
  #   info = @shopping_list.send("get_" + type + "_info", @package_id)
  #   happiness = @shopping_list.send("get_" + type +"_happiness", @package_id)
  #   result = "#{type_display}: #{info}: #{happiness} licks"
  #   happiness > 30 ? "* #{result}" : result
  # end
